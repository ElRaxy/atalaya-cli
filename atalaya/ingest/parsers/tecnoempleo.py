"""Parser de alertas Tecnoempleo.

Tecnoempleo envía emails desde `noreply@tecnoempleo.com` (y derivados). Los enlaces
a ofertas tienen el patrón `https://www.tecnoempleo.com/<slug>/<tech>/rf-<hash>`.
"""
...
_OFFER_HREF: Final = re.compile(
    r"https?://(?:www\.)?tecnoempleo\.com/[a-z0-9-]+/[a-z0-9-]+/rf-[a-z0-9]+",
    re.IGNORECASE,
)
...
class TecnoempleoAlertParser(BaseAlertParser):
    name: ClassVar[str] = "email_tecnoempleo"
    sender_patterns: ClassVar[list[str]] = [
        "noreply@tecnoempleo.com",
        "no-reply@tecnoempleo.com",
        "alertas@tecnoempleo.com",
        "@tecnoempleo.com",
    ]
    ...
    def _parse_html(html: str) -> list[IngestedOffer]:
        ...
        for anchor in tree.css("a"):
            ...
            if not title or len(title) < 3:
                continue
            if title.lower() in {"ver oferta", "más info", "mas info", "detalle"}:
                continue
            seen.add(url)
            ...
            company = TecnoempleoAlertParser._extract_company(block_text)
            location = TecnoempleoAlertParser._extract_location(block_text)
            ...
            offers.append(
                IngestedOffer(
                    title=title[:200],
                    company=company or "Tecnoempleo (sin empresa)",
                    url=url,
    ...
    def _extract_company(text: str) -> str:
        ...
        # Tecnoempleo a veces lista "<empresa> · <localidad>"
        match = re.search(
            r"\b([A-Z][A-Za-zÁÉÍÓÚÑ0-9& .,'-]{2,40})\s*·\s*[A-ZÁÉÍÓÚÑ]", text
        )
        if match:
            return match.group(1).strip(" .,")
        return ""
    ...
    def _extract_location(text: str) -> str:
        match = re.search(
            r"\b(?:Localidad|Provincia|Ubicación)[:\s]+([^|·\n]{2,60})", text
        )
        if match:
            return match.group(1).strip(" .,")
        return ""
    ...
    def _parse_text(text: str) -> list[IngestedOffer]:
        ...
        for match in _OFFER_HREF.finditer(text):
            ...
            offers.append(
                IngestedOffer(
                    title="Tecnoempleo offer",
                    company="Tecnoempleo (sin empresa)",
                    url=url,
                    location="España",
                    remote=False,
                )
